      ...
      <div className="admin-shell">
        <aside className="admin-sidebar">
          ...
          <div className="admin-bottom-actions">
            ...
            <button className="text-link" onClick={() => signOut(auth)}>
              Logout
            </button>
          </div>
        </aside>
        <main className="admin-main">
          <header className="admin-topbar">
            <div>
              <strong>KROON LUXE</strong>
              <span>Store dashboard</span>
            </div>
            <div>
              <button aria-label="Notifications" className="admin-icon-button">
                *
              </button>
              <span className="admin-avatar">{user?.email?.slice(0, 1).toUpperCase() ?? "A"}</span>
              <span className="admin-user-email">{user?.email ?? "Administrator"}</span>
            </div>
          </header>
          {children}
        </main>
