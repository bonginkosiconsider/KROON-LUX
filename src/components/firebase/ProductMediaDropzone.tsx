...
function isAllowedProductImage(file: File) {
  ...
  const allowedExtensions = new Set(["png", "jpg", "jpeg", "webp", "gif", "bmp", "heic", "heif", "avif", "svg"]);

  const isMimeImage = mimeType.startsWith("image/");
  const hasRecognizedExtension = allowedExtensions.has(extension);

  return (isMimeImage || hasRecognizedExtension) && file.size <= maxBytes;
}
...
export function ProductMediaDropzone({ media, onAddFiles, onRemove, onReorder }: ProductMediaDropzoneProps) {
  ...
  const accept = (items: File[]) => {
    const valid = items.filter((file) => isAllowedProductImage(file));
  ...
  return <div className="product-media-manager">
    <div className={`media-dropzone${isDraggingFiles ? " is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setIsDraggingFiles(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setIsDraggingFiles(false)} onDrop={dropFiles} onClick={() => input.current?.click()} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") input.current?.click(); }}>
      <input ref={input} className="sr-only" type="file" accept="image/*" multiple onChange={(event) => { accept(Array.from(event.target.files ?? [])); event.currentTarget.value = ""; }} />
      <strong>Add product media</strong><span>Drag product images here, or click to browse</span><small>Maximum 5 MB per image</small>
    </div>
