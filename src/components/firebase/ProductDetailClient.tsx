...
export function ProductDetailClient({ slug }: { slug: string }) {
  ...
  const activeGalleryUrl = galleryUrls[hoverImageIndex] ?? galleryUrls[0];

  const handleGalleryHover = () => {
    if (galleryUrls.length <= 1) return;
    setHoverImageIndex((current) => (current + 1) % galleryUrls.length);
  };

  return <main className="page-shell"><section className="product-detail"><div className="product-gallery">{galleryUrls.length ? (
    <div className="product-gallery-main" onMouseEnter={handleGalleryHover} onMouseLeave={() => setHoverImageIndex(0)}>
      <img src={activeGalleryUrl} alt={`${product.title} gallery view`} />
    </div>
